...
RUN apt-get update \
  ...
  && apt-get install -y --no-install-recommends -o DPkg::Options::=--force-confold netcat-traditional pkg-config default-libmysqlclient-dev libmariadbclient-dev libpq-dev build-essential libldap2-dev libsasl2-dev ldap-utils git
